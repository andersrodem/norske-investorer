...
async function getInvestorTypeId(name: string) {
  const investorType = await prisma.investorType.findUnique({ where: { name } })
  if (!investorType) throw new Error(`InvestorType with name "${name}" not found`)
...
async function main() {
  ...
  for (const investor of data) {
    ...
    const investorTypeIds = await Promise.all(investor.investorTypes.map(getInvestorTypeId))
    ...
    await prisma.investor.create({
      data: {
        ...
        engagementId: investor.engagementId,
        phases: {
